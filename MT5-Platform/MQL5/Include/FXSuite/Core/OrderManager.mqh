...
public:
                   ...
                   const string signal_id, OrderRecord &rec)
   {
      ...
      bool ok = (type==ORDER_TYPE_BUY)
                  ? m_trade.Buy(lots, symbol, 0.0, sl, tp, signal_id)
                  : m_trade.Sell(lots, symbol, 0.0, sl, tp, signal_id);
                 ...
                 const string signal_id, OrderRecord &rec)
   {
      ...
      if(type==ORDER_TYPE_BUY_STOP)
         ok = m_trade.BuyStop(lots, symbol, price, sl, tp, ORDER_TIME_GTC, 0);   // 8 params (no comment)
      else if(type==ORDER_TYPE_SELL_STOP)
         ok = m_trade.SellStop(lots, symbol, price, sl, tp, ORDER_TIME_GTC, 0);
      rec.ticket = m_trade.ResultOrder(); rec.fill_price = 0.0; rec.reason = ok?SLIP_MARKET:SLIP_REJECT;
      return ok;
   }

   bool ModifySLTP(const ulong position_ticket, const double sl, const double tp)
   {
   ...
   {
      return m_trade.OrderDelete((long)order_ticket);
   }
};