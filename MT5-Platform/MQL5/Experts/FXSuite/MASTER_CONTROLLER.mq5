...
void OnTimer()
{
   ...
   // news blackout for execution safety
   ...
   // overwrite feature[21] = minutes-to-next-high from calendar.csv
   ...
   // probability → size: fractional "kelly-like" multiplier using simple RR
   double rr = (double)InpTP_Pips / (double)InpSL_Pips; if(rr<=0.0) rr=1.0;
   double edge = pwin - (1.0-pwin)/rr;
   double prob_mult = MathMax(0.50, MathMin(1.50, 0.25 + 0.5*edge/0.10)); // gentle map

   double lots = base_lots * rp.lot_mult * prob_mult;

   // portfolio guardrails
   string reason;
   double assumed_risk_pct = InpRiskPct; // approximate: we size by SL so this holds
...
// --- helpers (kept from your previous version) ---
double PipValue()
{
...
{
   // map symbol to currency code (first or second leg)
   string ccy = StringSubstr(symbol,0,3); // simple: take base
   // open CSV
   ...
   while(!FileIsEnding(h)){
      ...
      if(ts >= now_utc && StringFind(StringUpper(impact), "HIGH")>=0 && (cur==ccy || cur=="ALL")){
